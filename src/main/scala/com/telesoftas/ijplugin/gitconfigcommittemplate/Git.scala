...
object Git {
  ...
  def commitTemplate(project: Project): Option[String] = {
    ...
    project.repoRoot.flatMap { repoRoot =>
      val config = new GitLineHandler(project, repoRoot, GitCommand.CONFIG).withParameters("commit.template")
      Try(git.runCommand(config)).toOption.flatMap(_.getOutput.asScala.headOption).map(new File(_).read)
    }
  }
